# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).


## [1.5.0](https://github.com/raul-izquierdo/solutions/compare/v1.4.3...v1.5.0) - 2026/02/23

### BREAKING CHANGES

### Added

- Improved error messages when github fails.
- Improved feedback when fetching groups and solutions to help identify issues.

## [1.4.3]

### Fixed

- Only the first 100 repositories were being considered when detecting solutions, which could lead to missing solutions if there are more than 100 repositories in the organization. This has been fixed to consider all repositories.

## [1.4.2]

### Fixed

- In version 1.4.1 solutions were displayed in sorted order, but the mapping to the actual solution names was incorrect.

## [1.4.1]

### Fixed

- Fixed an issue where the program would exit with a non-zero status code when no groups or solutions were found, which could be misinterpreted as an error in automated environments.
- Solutions are now displayed in sorted order in the manual selection prompt for better usability.

## [1.4.0]

### Added

- Added `--dry-run` option to preview changes without applying them.

### Changed

- Improved startup time by only making the necessary GitHub connections for the selected group.


## [1.3.0]

### Added

- Added `-r` option to specify a regular expression for detecting solution repositories. Default is `.*solution$` (repositories whose names end with `solution`).

### Fixed

- Fixed file permissions in Linus scripts.
- Fixed duplicated "(y/N)" in some prompts.

## [1.2.0]

### Changed

- Improved user prompts in automatic selection mode.

## [1.1.0]

### Added

- Error messages now include the organization name to help identify issues when working with the wrong organization.
- The schedule file name now defaults to `schedule.csv` instead of requiring it to be specified with `-s`.

### Changed

- Groups in `schedule.csv` are no longer required to exist as teams, allowing use of the full `schedule.csv` even when some groups don't have their teams created yet.

### Fixed

- When there are no teams or solutions, the app now warns and stops instead of showing a confusing error message.

## [1.0.0]


### Added

- Automatic granting of access to solutions based on a schedule.
- Manual selection of group and solution when automatic detection is not possible.
